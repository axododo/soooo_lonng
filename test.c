...
void init_image(t_env *env) {
  ...
  env->tile_w = w;
   env->tile_h = h;
  env->img_wall = mlx_new_image_from_file(env->mlx, "textures/wall.png", &w, &h);
  if (!env->img_wall){
    write(2, "Error: cannot load wall.png\n", 28);
    return;}
  env->tile_w = w;
  env->tile_h = h;
  env->img_floor = mlx_new_image_from_file(env->mlx, "textures/floor.png", &w, &h);
  if (!env->img_floor){
    write(2, "Error: cannot load floor.png\n", 28);
    return;}
  env->img_collect = mlx_new_image_from_file(env->mlx, "textures/collect.png", &w, &h);
  if (!env->img_collect){
    write(2, "Error: cannot load collect.png\n", 28);
    return;}
  env->img_exit = mlx_new_image_from_file(env->mlx, "textures/exit.png", &w, &h);
  if (!env->img_exit){
    write(2, "Error: cannot load exit.png\n", 28);
    return;}
  env->img_player = mlx_new_image_from_file(env->mlx, "textures/player.png", &w, &h);
  if (!env->img_player){
    write(2, "Error: cannot load player.png\n", 28);
    return;}
    env->img_win = mlx_new_image_from_file(env->mlx, "textures/win.png", &w, &h);
    if (!env->img_win){
  ...
  void render_map(t_env *env)
{
    ...
    while (y < env->map->height)
    {
        ...
        {
            char c = env->map->grid[y][x];
            if (c == 'P')
              mlx_put_image_to_window(env->mlx, env->win, env->img_player, x * env->tile_w, y * env->tile_h);
            else if (c == 'X') {
              mlx_put_image_to_window(env->mlx, env->win, env->img_exit, x * env->tile_w, y * env->tile_h);
            }
            else if (c == 'C')
              mlx_put_image_to_window(env->mlx, env->win, env->img_collect, x * env->tile_w, y * env->tile_h);
            else if (c == '1') {
              mlx_put_image_to_window(env->mlx, env->win, env->img_wall, x * env->tile_w, y * env->tile_h);
            }
            else if (c == '0' || c == 'E')
              mlx_put_image_to_window(env->mlx, env->win, env->img_floor, x * env->tile_w, y * env->tile_h);
            x++;
        }
        y++;
    }
...
void win(t_env *env)  {
  ...
  int win_h = env->map->height * env->tile_h;
  int img_w = 480;
  int img_h = 320;
  env->stWin = 1;

  mlx_clear_window(env->mlx, env->win, (mlx_color){ .rgba = 0x000000FF });
  mlx_put_image_to_window(env->mlx, env->win, env->img_win,
